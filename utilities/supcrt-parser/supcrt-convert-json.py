...
def parseAqueousData(f):
    ...
    hkf["a2"] = float(str(float(words[1]) * 1.0e02))
    hkf["a3"] = float(str(float(words[2]) * 1.0e00))
    hkf["a4"] = float(str(float(words[3]) * 1.0e04))

    # Parse the 6th line of the aqueous species data
    words = f.readline().strip().split()
    hkf["c1"] = float(str(float(words[0]) * 1.0e00))
    hkf["c2"] = float(str(float(words[1]) * 1.0e04))
    hkf["wref"] = float(str(float(words[2]) * 1.0e05))
...
def parseGaseousData(f):
    ...
    words = f.readline().strip().split()
    maierkelly["a"] = float(str(float(words[0]) * 1.0e00))
    maierkelly["b"] = float(str(float(words[1]) * 1.0e-03))
    maierkelly["c"] = float(str(float(words[2]) * 1.0e05))
...
def parseMineralData(f, numPhaseTransitions):
    ...
    if numPhaseTransitions == 0:
        ...
        for i in range(numPhaseTransitions + 1):
            words = f.readline().strip().split()
            maierkelly["a"].append(float(str(float(words[0]) * 1.0e00)))
            maierkelly["b"].append(float(str(float(words[1]) * 1.0e-03)))
            maierkelly["c"].append(float(str(float(words[2]) * 1.0e05)))
...
def correctMineralData(data):
    ...
    if maierkelly["Gf"] == 999999.0:
        maierkelly["Gf"] = None
    if maierkelly["Hf"] == 999999.0:
        maierkelly["Hf"] = None

    if maierkelly["numPhaseTransitions"] > 0:
        for i in range(maierkelly["numPhaseTransitions"]):
            if maierkelly["Htr"][i] == 999999.0:
                maierkelly["Htr"][i] = None
            if maierkelly["Vtr"][i] == 999999.0:
                maierkelly["Vtr"][i] = None
            if maierkelly["dPdTtr"][i] == 999999.0:
                maierkelly["dPdTtr"][i] = None
...
def main():
    ...
    for s in datalist["species"]:
        ...
        if elements.get("Z", 0) != s.get("charge", 0):
            print "{} has charge {} but Z {}".format(
                s["name"], s.get("charge", 0), elements.get("Z", 0)
            )
