...
def correctAqueousData(data):
    ...
    if data.references == "PC2":
        data.name = (
            ...
            .replace("4-", "----")
            .replace("1", "+")
        )  # necessary to change d+H3GDP1 to d+H3GDP+
...
def correctMineralData(data):
    ...
    data.name = (
        data.name.replace("-Ord", ",ord").replace("-Dis", ",dis").replace(",Ordered", ",ord")
    )
    ...
    if data.nptrans == 0:
        ...
        for i in range(data.nptrans + 1):
            ...
            if i < data.nptrans:
                ...
                temperature_range["Htr"] = (
                    createValueUnitsDict(data.Htr[i], "cal/mol") if data.Htr[i] != None else None
                )
                temperature_range["Vtr"] = (
                    createValueUnitsDict(data.Vtr[i], "cm3/mol") if data.Vtr[i] != None else None
                )
                temperature_range["dPdTtr"] = (
                    createValueUnitsDict(data.dPdTtr[i], "bar/K")
                    if data.dPdTtr[i] != None
                    else None
                )
...
if len(aqueous_datalist) + len(gaseous_datalist) + len(mineral_datalist) != len(datalist):
    raise RuntimeError(
        "the number of aqueous, gaseous and mineral species does not match the total number of species"
    )
