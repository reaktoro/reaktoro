...
#include "ThermoStateSpeciesHKF.hpp"
...
#include <Reaktor/Thermodynamics/ElectroStateSpecies.hpp>
#include <Reaktor/Thermodynamics/ElectroStateSpeciesHKF.hpp>
#include <Reaktor/Thermodynamics/ThermoStateSpecies.hpp>
#include <Reaktor/Thermodynamics/WaterConstants.hpp>
#include <Reaktor/Thermodynamics/ElectroStateWater.hpp>
...
auto speciesThermoHKF(double T, double P, const ThermoStateWater& wt) -> ThermoStateSpecies
{
...
auto speciesThermoHKF(double T, double P, const AqueousSpecies& species, const ElectroStateSpecies& se, const ElectroStateWater& we) -> ThermoStateSpecies
{
    // Get the HKF thermodynamic data of the species
    const auto& hkf = species.thermoData().hkf.get();
...
auto speciesThermoHKF(double T, double P, const AqueousSpecies& species) -> ThermoStateSpecies
{
	ThermoStateWater wt = waterThermo(T, P, WagnerPruss);

	if(species.name() == "H2O(l)")
		return speciesThermoHKF(T, P, wt);

    ElectroStateWater we = waterElectro(T, P, wt);

    FunctionG g(T, P, wt);

    ElectroStateSpecies se = speciesElectro(g, species);

    return speciesThermoHKF(T, P, species, se, we);
}

auto speciesThermoHKF(double T, double P, const GaseousSpecies& species) -> ThermoStateSpecies
{
...
auto speciesThermoHKF(double T, double P, const MineralSpecies& species) -> ThermoStateSpecies
{
