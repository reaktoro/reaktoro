...
// Reaktor includes
#include <Reaktor/Thermodynamics/ElectroStateWaterJohnsonNorton.hpp>
#include <Reaktor/Thermodynamics/ThermoStateWater.hpp>

namespace Reaktor {

ElectroStateWater::ElectroStateWater()
: epsilon(0), epsilonT(0), epsilonP(0), epsilonTT(0), epsilonTP(0), epsilonPP(0),
  bornZ(0), bornY(0), bornQ(0), bornN(0), bornU(0), bornX(0)
{}

auto operator<<(std::ostream& out, const ElectroStateWater& we) -> std::ostream&
{
...
auto waterElectro(double T, double P, const ThermoStateWater& wt) -> ElectroStateWater
{
