...
auto gaseousActivitySpycherReedH2OCO2CH4(const GaseousActivityParams& params, Index iH2O, Index iCO2, Index iCH4) -> std::vector<PartialScalar>
{
    ...
    for(int i = 0; i < 3; ++i)
    {
        if(iH2O < num_species) grad(phiH2O)[iH2O] = dphi[0][0];
        if(iCO2 < num_species) grad(phiH2O)[iCO2] = dphi[0][1];
        if(iCH4 < num_species) grad(phiH2O)[iCH4] = dphi[0][2];

        if(iH2O < num_species) grad(phiCO2)[iH2O] = dphi[1][0];
        if(iCO2 < num_species) grad(phiCO2)[iCO2] = dphi[1][1];
        if(iCH4 < num_species) grad(phiCO2)[iCH4] = dphi[1][2];

        if(iH2O < num_species) grad(phiCH4)[iH2O] = dphi[2][0];
        if(iCO2 < num_species) grad(phiCH4)[iCO2] = dphi[2][1];
        if(iCH4 < num_species) grad(phiCH4)[iCH4] = dphi[2][2];
    }
