...
public:
    ...
    auto setSpecificSurfaceArea(double value, const std::string& unit) -> void
    {
        ...
        specific_surface_area$ = 0.0 * unit(m2)/unit(g);
        volumetric_surface_area$ = 0.0 * unit(m2)/unit(m3);
...
auto MineralReaction::setEquilibriumConstant(const EquilibriumConstantFn& equilibrium_constant) -> MineralReaction&
{
...
auto MineralReaction::equilibriumConstant() const -> const EquilibriumConstantFn&
{
