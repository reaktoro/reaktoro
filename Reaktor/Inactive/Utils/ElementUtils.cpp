...
auto atomicMass(std::string element) -> units::MolarMass
{
    ...
    auto atomic_mass = entry->second;
    return atomic_mass * unit(g)/unit(mol);
}

auto molarMass(const std::map<std::string, double>& elements) -> units::MolarMass
{
    units::MolarMass molar_mass = 0.0 * unit(g)/unit(mol);
