...
// Reaktor includes
#include <Reaktor/Core/ChemicalConverter.hpp>
...
#include <Reaktor/Reactions/MineralReaction.hpp>
...
private:
    ...
    /// The chemical converter instance
    ...
    std::map<std::string, ChemicalPotentialFn> chemical_potentials;

public:
    explicit Impl(const Database& database)
    : database(database), converter(database)
    {}
    ...
    auto addAqueousPhase(const std::vector<AqueousSpecies>& species) -> AqueousPhase&
    {
        ...
        aqueous_phase = AqueousPhase(species);
    ...
    auto addAqueousPhase(const std::vector<std::string>& species) -> AqueousPhase&
    {
        ...
        return addAqueousPhase(aqueous_species);
    }
    ...
    auto addAqueousPhaseAllSpecies() -> AqueousPhase&
    {
        return addAqueousPhase(database.aqueousSpecies());
    }

    auto addGaseousPhase(const std::vector<GaseousSpecies>& species) -> GaseousPhase&
    {
        // Create the gaseous phase
        gaseous_phase = GaseousPhase(species);
    ...
    auto addGaseousPhase(const std::vector<std::string>& species) -> GaseousPhase&
    {
        ...
        return addGaseousPhase(gaseous_species);
    }
    ...
    auto addGaseousPhaseAllSpecies() -> GaseousPhase&
    {
        return addGaseousPhase(database.gaseousSpecies());
    }

    auto addMineralPhase(const std::vector<MineralSpecies>& species) -> MineralPhase&
    {
        mineral_phases.push_back(MineralPhase(species));
        return mineral_phases.back();
    }
    ...
    auto addMineralPhase(const std::vector<std::string>& species) -> MineralPhase&
    {
        ...
        return addMineralPhase(mineral_species);
    }
    ...
    {
        chemical_potentials[species] = potential;
    }
    ...
    {
        Phase converted = converter.convertPhase(phase);
...
auto ChemicalEditor::addAqueousPhase(const std::vector<AqueousSpecies>& species) -> AqueousPhase&
{
...
auto ChemicalEditor::addAqueousPhase() -> AqueousPhase&
{
    return pimpl->addAqueousPhaseAllSpecies();
}

auto ChemicalEditor::addGaseousPhase() -> GaseousPhase&
{
    return pimpl->addGaseousPhaseAllSpecies();
}

auto ChemicalEditor::addGaseousPhase(const std::vector<GaseousSpecies>& species) -> GaseousPhase&
{
...
auto ChemicalEditor::addMineralPhase(const std::vector<MineralSpecies>& species) -> MineralPhase&
{
...
auto ChemicalEditor::setChemicalPotentialFn(const std::string& species, const ChemicalPotentialFn& potential) -> void
{
	pimpl->setChemicalPotentialFn(species, potential);
}
