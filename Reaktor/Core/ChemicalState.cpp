...
auto ChemicalState::set(unsigned index, double amount) -> void
{
    ...
    Assert(index < num_species, "Cannot set the molar amount of the species with an out-of-range index.", "");
...
auto ChemicalState::set(std::string species, double amount, std::string units) -> void
{
...
//         return (element == "H") ? (ne - 2*nw)/mw : (element == "O") ? (ne - nw)/mw : ne/mw;
//     }
...
//     {
//         return (ispecies > system.numSpecies()) ? 0.0 : n[ispecies] * unit(mol);
//     }
