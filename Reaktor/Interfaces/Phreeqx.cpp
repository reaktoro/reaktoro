...
auto Phreeqx::indexElement(std::string name) const -> unsigned
{
...
auto Phreeqx::indexPhase(std::string name) const -> unsigned
{
    return index(name, pimpl->phase_names);
}

auto Phreeqx::indexPhaseWithSpecies(unsigned ispecies) const -> unsigned
{
    unsigned counter = 0;
...
{
    Element element;
    element.setName(phreeqx.elementName(ielement));
    element.setMolarMass(phreeqx.elementMolarMass(ielement));
    return element;
}
...
auto createPhases(const Phreeqx& phreeqx) -> std::vector<Phase>
{
    ...
    {
        Phase phase;
        phase.setName(phreeqx.phaseName(iphase));

        std::vector<Species> species;
        for(unsigned i = offset; i < offset + phreeqx.numSpeciesInPhase(iphase); ++i)
            species.push_back(createSpecies(phreeqx, i));
...
Phreeqx::operator ChemicalSystem() const
{
    ...
    model.standard_gibbs_energy_fn = [=](double T, double P) mutable -> ThermoVector
    {
        ...
        res.val = phreeqx.standardGibbsEnergies();
        return res;
    };
    ...
    model.standard_volume_fn = [=](double T, double P) mutable -> ThermoVector
    {
        ...
        phreeqx.setPressure(P);
        ThermoVector res(num_species);
        res.val = phreeqx.standardVolumes();
        return res;
    };
    ...
    model.chemical_potential_fn = [=](double T, double P, const Vector& n) mutable -> ChemicalVector
    {
        ...
        res.val = phreeqx.chemicalPotentials();
        return res;
    };
    ...
    model.phase_molar_volume_fn = [=](double T, double P, const Vector& n) mutable -> ChemicalVector
    {
        ...
        res.val = phreeqx.phaseMolarVolumes();
        return res;
    };
