...
auto nsteps_cb = 45;  // the number of steps in the reactive transport simulation of the first injection phase
auto nsteps_sw = 855;  // the number of steps in the reactive transport simulation of the second injection phase
...
int main()
{
    ...
    params.nsteps = nsteps_cb + nsteps_sw;     // the total number of steps in the reactive transport simulation
...
auto runReactiveTransport(ReactiveTransportParams& params, ReactiveTransportResults& results) -> void
{
    ...
    auto folder = params.makeResultsFolder("scaling-seq-cb-sw");
    ...
    if(params.use_smart_equilibrium_solver) {
        ...
        std::cout << "smart equilibrium acceptance rate (cb) : " << results.smart_equilibrium_acceptance_rate << " / "
                  << (1 - results.smart_equilibrium_acceptance_rate) * params.ncells * nsteps_cb
                  << " fully evaluated GEMS out of " << params.ncells * nsteps_cb  << std::endl;
    ...
    if(params.use_smart_equilibrium_solver) {
        ...
        std::cout << "smart equilibrium acceptance rate (sw) : " << results.smart_equilibrium_acceptance_rate << " / "
                  << (1 - results.smart_equilibrium_acceptance_rate) * params.ncells * nsteps_sw
                  << " fully evaluated GEMS out of " << params.ncells * nsteps_sw  << std::endl;
