...
solution.setActivityModel(chain(ActivityModelHKF(), ActivityModelElectrostatics(ActivityModelDDLParams())))
...
system = ChemicalSystem(db, solution, complexation_phase_Hfo_w, complexation_phase_Hfo_s)
...
print(f"  pH   x(Hfo_sOH) x(Hfo_sOH2+)   x(Hfo_sO-)   x(Hfo_wOH) x(Hfo_wOH2+)   x(Hfo_wO-) succeed")
for pH in pHs:
      ...
      print(f"{pH:4.1f} {xHfo_sOH:12.4e} {xHfo_sOH2:12.4e} {xHfo_sO:12.4e} {xHfo_wOH:12.4e} {xHfo_wOH2:12.4e} {xHfo_wO:12.4e} {res.optima.succeeded:6d}")
...
ax.legend(loc="best")
ax.grid()
plt.savefig(f"sorbed-fractions-h2o-vs-with-ebulk-pH-NaCl-{nNaCl}.png", bbox_inches='tight')
plt.close()