...
for i in range(N):
    ...
    equilibrate(state)
    data["P"].append(P/1e3)
    data["NaCl(l)"].append(float(state.speciesAmount("NaCl(l)")))
    data["Na(g)"].append(float(state.speciesAmount("Na(g)")))
    data["Cl(g)"].append(float(state.speciesAmount("Cl(g)")))
    data["Cl2(g)"].append(float(state.speciesAmount("Cl2(g)")))
    data["NaCl(g)"].append(float(state.speciesAmount("NaCl(g)")))
    data["Na2Cl2(g,ring)"].append(float(state.speciesAmount("Na2Cl2(g,ring)")))
    data["Na+(g)"].append(float(state.speciesAmount("Na+(g)")))
    data["Cl-(g)"].append(float(state.speciesAmount("Cl-(g)")))
