...
columns = ["pH",
           ...
           "xHfo_sOH", "xHfo_sOH2+", "xHfo_sO-", "xHfo_wOH", "xHfo_wOH2+", "xHfo_wO-", "sigma"]
...
print(f"  pH   x(Hfo_sOH) x(Hfo_sOH2+)   x(Hfo_sO-)   x(Hfo_wOH) x(Hfo_wOH2+)   x(Hfo_wO-)            I            Z        sigma")
for pH in pHs:
...
ax.set_title("Dependence of water sorption on pH (no EDL)")
ax.set_xlabel("pH")
ax.set_ylabel("Amount of sorbed species")
ax.legend(loc="best")
ax.grid()
plt.savefig(f"sorbed-amounts-h2o-vs-pH-no-edl.png", bbox_inches='tight')
plt.close()
...
ax.legend(loc="best")
ax.grid()
plt.savefig(f"sorbed-fractions-h2o-vs-pH-no-edl.png", bbox_inches='tight')
plt.close()
...
ax.set_xlabel("pH")
ax.set_ylabel(r"Surface charge density, C/m$^2$")
ax.legend(loc="best")
ax.grid()
plt.savefig(f"surface-charge-density-vs-pH-no-edl.png", bbox_inches='tight')
plt.close()