...
Once the initial and final equilibrium states have been calculated, it is now time to trace the reaction path between them, with each intermediate state in chemical equilibrium. For this, we use the class `EquilibriumPath` in Reaktoro. Note that its initialization requires an instance of class `ChemicalSystem`:
