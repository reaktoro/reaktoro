...
const String doc_elements_wrong = R"(
Elements:
  Symbol: A
  MolarMass: 1.0
)";

const String doc_species_wrong = R"(
Species:
  Name: A2B(l)
  Formula: A2B
  Elements: 2:A 1:B
...
{
    SECTION("Testing different formats, based on dictionary or list")
    {
        String doc = GENERATE(doc_dict_based, doc_list_based);
    ...
    SECTION("Testing non-conforming databases")
    {
        CHECK_THROWS(DatabaseParser(Data::parse(doc_elements_wrong)));
        CHECK_THROWS(DatabaseParser(Data::parse(doc_species_wrong)));
    }
}
