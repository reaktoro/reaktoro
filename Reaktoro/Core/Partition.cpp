...
{
    Indices indices;
    indices.reserve(system.species().size());
    for(Index iphase : iphases)
    {
        const auto species_in_phase = system.phases()[iphase].species();
        const Index ifirst = detail::resolveSpeciesIndex(system, species_in_phase[0].name()); //.indexFirstSpeciesInPhase(iphase);
        const Index nspecies = species_in_phase.size();
...
struct Partition::Impl
{
    ...
    auto setEquilibriumSpecies(const Indices& ispecies) -> void
    {
        ...
        indices_inert_species       = detail::difference(indices_inert_species, ispecies);
        indices_kinetic_species     = detail::difference(universe_species, detail::unify(ispecies, indices_inert_species));
        finalise();
    }
    ...
    auto setKineticSpecies(const Indices& ispecies) -> void
    {
        ...
        indices_inert_species       = detail::difference(indices_inert_species, ispecies);
        indices_equilibrium_species = detail::difference(universe_species, detail::unify(ispecies, indices_inert_species));
        finalise();
    }
    ...
    auto setInertSpecies(const Indices& ispecies) -> void
    {
        ...
        indices_equilibrium_species = detail::difference(indices_equilibrium_species, ispecies);
        indices_kinetic_species     = detail::difference(indices_kinetic_species, ispecies);
    ...
    auto setFluidPhases(const Indices& indices) -> void
    {
        ...
        indices_solid_phases = detail::difference(universe_phases, indices);
        finalise();
    }
    ...
    auto setSolidPhases(const Indices& indices) -> void
    {
        ...
        indices_fluid_phases = detail::difference(universe_phases, indices);
        finalise();
    }
    ...
    auto finalise() -> void
    {
        ...
        indices_equilibrium_fluid_species = detail::intersect(indices_equilibrium_species, indices_fluid_species);
        indices_equilibrium_solid_species = detail::intersect(indices_equilibrium_species, indices_solid_species);

        indices_kinetic_fluid_species = detail::intersect(indices_kinetic_species, indices_fluid_species);
        indices_kinetic_solid_species = detail::intersect(indices_kinetic_species, indices_solid_species);

        indices_inert_fluid_species = detail::intersect(indices_inert_species, indices_fluid_species);
        indices_inert_solid_species = detail::intersect(indices_inert_species, indices_solid_species);
...
auto Partition::formulaMatrixEquilibriumPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_equilibrium;
}

auto Partition::formulaMatrixEquilibriumFluidPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_equilibrium_fluid;
}

auto Partition::formulaMatrixEquilibriumSolidPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_equilibrium_solid;
}

auto Partition::formulaMatrixKineticPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_kinetic;
}

auto Partition::formulaMatrixKineticFluidPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_kinetic_fluid;
}

auto Partition::formulaMatrixKineticSolidPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_kinetic_solid;
}

auto Partition::formulaMatrixInertPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_inert;
}

auto Partition::formulaMatrixInertFluidPartition() const -> MatrixXdConstRef
{
    return pimpl->formula_matrix_inert_fluid;
}

auto Partition::formulaMatrixInertSolidPartition() const -> MatrixXdConstRef
{
...
auto operator==(const Partition& l, const Partition& r) -> bool
{
    ...
    return
        l.indicesEquilibriumSpecies() == r.indicesEquilibriumSpecies() &&
        l.indicesKineticSpecies() == r.indicesKineticSpecies() &&
        l.indicesInertSpecies() == r.indicesInertSpecies();
}
