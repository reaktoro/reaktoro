...
//            std::cout << "quantity.value(word).val() = " << quantity.value(word) << std::endl;

            auto space = spacings[icolumn];
            auto val = (word == "i") ? real(iteration) : quantity.value(word);
            if(datafile.is_open())
    ...
    auto update(const ChemicalState& state, const ChemicalProps& properties, double t) -> void
    {
        // Output values on a new line
        ...
        {
            auto space = spacings[icolumn];
            auto val = (word == "i") ? iteration : quantity.value(word).val();
...
auto ChemicalOutput::update(const ChemicalState& state, const ChemicalProps& properties, double t) -> void
{
