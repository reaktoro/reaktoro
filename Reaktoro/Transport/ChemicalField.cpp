...
struct ChemicalField::Impl
{
    ...
    /// The chemical states in the chemical field.
    std::vector<ChemicalProps> m_properties;

    /// Construct a ChemicalField::Impl instance.
    Impl(Index size, const ChemicalSystem& system)
    : m_size(size), m_system(system), m_states(size, ChemicalState(system)), m_properties(size, ChemicalProps(system))
    {
    }

    /// Construct a ChemicalField::Impl instance.
    Impl(Index size, const ChemicalState& state)
    : m_size(size), m_system(state.system()), m_states(size, state), m_properties(size, ChemicalProps(state))
    {
    }
...
auto ChemicalField::temperature(ArrayXr values) -> void
{
    pimpl->temperature(values);
}

auto ChemicalField::pressure(ArrayXr values) -> void
{
    pimpl->pressure(values);
}

auto ChemicalField::elementAmounts(ArrayXr values) -> void
{
...
auto ChemicalField::properties() -> std::vector<ChemicalProps>&
{
