...
inline void checkTemperatureUnit(const string& symbol)
{
    if(!temperatureUnitsMap.count(symbol)) {
        stringstream error;
        error << "*** Error *** there is no such temperature unit named: " << symbol << ".";
...
inline void checkDerivedUnit(const string& symbol)
{
    if(!derivedUnitsMap.count(symbol)) {
        stringstream error;
        error << "*** Error *** there is no such unit named: " << symbol << ".";
        throw std::runtime_error(error.str());
    }
}
...
double factor(const string& symbol)
{
    ...
    if(units.front().symbol == symbol)
        return pow(units.front().factor, units.front().power);
    double res = 1.0;
...
{
    if(temperatureUnitsMap.count(symbol)) {
        res[symbol] += multpower;
        return;
    }
    checkDerivedUnit(symbol);
    const auto& units = derivedUnitsMap[symbol];
    if(units.front().symbol == symbol)
        res[units.front().symbol] += units.front().power * multpower;
    else
        for(const auto& unit : units)
...
struct Node
{
    ...
    Node(string str)
        : str(str), left(nullptr), right(nullptr) {}
...
inline void checkConvertibleUnits(const DerivedUnit& from, const DerivedUnit& to, const string& strfrom, const string& strto)
{
    if(dimension(from) != dimension(to)) {
        stringstream error;
        error << "*** Error *** the dimensions of the units " << strfrom << " and " << strto << " do not match.";
        throw std::runtime_error(error.str());
    }
}
