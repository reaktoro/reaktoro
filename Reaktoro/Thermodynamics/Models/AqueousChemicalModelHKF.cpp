...
auto computeAqueousActivityHKFCharged(const AqueousMixtureState& state, Index ispecies, Index iwater, double charge, double eff_radius) -> ChemicalScalar
{
    ...
    const double z  = charge;
    const double z2 = z*z;

    // The absolute Born coefficient of the charged species
...
auto computeAqueousActivityHKFWater(const AqueousMixtureState& state, Index iwater, const std::vector<double>& charges, const std::vector<double>& eff_radii) -> ChemicalScalar
{
    ...
    // The stoichiometric molalities of the ionic species and their molar derivatives
    ...
    for(unsigned ion = 0; ion < num_ions; ++ion)
    {
        ...
        const double z  = charges[ion];
        const double zz = z*z;

        // The effective electrostatic radius of the current ion
        const double eff_radius = eff_radii[ion];

        // The Born coefficient of the current ion
        const double omega = eta*zz/eff_radius - z*omegaH;
...
auto aqueousChemicalModelHKF(const AqueousMixture& mixture) -> PhaseChemicalModel
{
    ...
    PhaseChemicalModel f = [=](double T, double P, const Vector& n)
    {
        ...
        for(unsigned i = 0; i < num_charged_species; ++i)
        {
            ...
            // The log10 of the activity coefficient of the charged species (in molar fraction scale) and its molar derivatives
            ChemicalScalar log10_gi = -(A*z2*sqrtI)/lambda + (omega_abs * bNaCl + bNapClm - 0.19*(std::abs(z) - 1.0)) * I;
