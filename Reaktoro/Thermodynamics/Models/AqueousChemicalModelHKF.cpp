...
const std::map<std::string, double> effective_radii =
    {
        {"H+", 3.08}, {"Fe+++", 3.46}, {"Li+", 1.64}, {"Al+++", 3.33}, {"Na+", 1.91}, {"Au+++", 3.72}, {"K+", 2.27}, {"La+++", 3.96}, {"Rb+", 2.41}, {"Gd+++", 3.79}, {"Cs+", 2.61}, {"In+++", 3.63}, {"NH4+", 2.31}, {"Ca+++", 3.44}, {"Ag+", 2.20}, {"F-", 1.33}, {"Au+", 2.31}, {"Cl-", 1.81}, {"Cu+", 1.90}, {"Br-", 1.96}, {"Mg++", 2.54}, {"I-", 2.20}, {"Sr++", 3.00}, {"OH-", 1.40}, {"Ca++", 2.87}, {"HS-", 1.84}, {"Ba++", 3.22}, {"NO3-", 2.81}, {"Pb++", 3.08}, {"HCO3-", 2.10}, {"Zn++", 2.62}, {"HSO4-", 2.37}, {"Cu++", 2.60}, {"ClO4-", 3.59}, {"Cd++", 2.85}, {"ReO4-", 4.23}, {"Hg++", 2.98}, {"SO4--", 3.15}, {"Fe++", 2.62}, {"CO3--", 2.81}, {"Mn++", 2.68}};
...
auto effectiveIonicRadius(const AqueousSpecies& species) -> double
{
    ...
    if(Zi == -1)
        return 1.81; // based on Cl- value
    if(Zi == -2)
        return 3.00; // based on rounded average of CO3-- and SO4-- values
    if(Zi == -3)
        return 4.20; // based on estimation from straight line fit with charge
    if(Zi == +1)
        return 2.31; // based on NH4+ value
    if(Zi == +2)
        return 2.80; // based on rounded average for +2 species in the HKF table of effective ionic radii
    if(Zi == +3)
        return 3.60; // based on rounded average for +3 species in the HKF table of effective ionic radii
    if(Zi == +4)
        return 4.50; // based on estimaton using HKF eq. 142
    if(Zi < -3)
        return -Zi * 4.2 / 3.0; // based on linear extrapolation
...
auto aqueousChemicalModelHKF(const AqueousMixture& mixture) -> PhaseChemicalModel
{
    ...
    PhaseChemicalModel model = [=](PhaseChemicalModelResult& res, Temperature T, Pressure P, VectorConstRef n) mutable {
        ...
        for(unsigned i = 0; i < num_charged_species; ++i) {
            ...
            const double z = charges[i];
            const double z2 = z * z;

            // The effective radius of the charged species
            const double eff_radius = effective_radii[i];

            // The Born coefficient of the current charged species
            const double omega = eta * z2 / eff_radius - z * omegaH;

            // The absolute Born coefficient of the current charged species
            const double omega_abs = eta * z2 / eff_radius;

            // The Debye-Huckel ion size parameter of the current ion as computed by Reed (1982) and also in TOUGHREACT
            const double a = (z < 0) ? 2.0 * (eff_radius + 1.91 * std::abs(z)) / (std::abs(z) + 1.0) : 2.0 * (eff_radius + 1.81 * std::abs(z)) / (std::abs(z) + 1.0);
        ...
        if(xw != 1.0)
            res.ln_activities[iwater] = ln10 * Mw * phi;
        else
            res.ln_activities[iwater] = ln_xw;
