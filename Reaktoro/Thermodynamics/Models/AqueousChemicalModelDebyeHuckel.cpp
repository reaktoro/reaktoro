...
auto aqueousChemicalModelDebyeHuckel(const AqueousMixture& mixture, const DebyeHuckelParams& params)-> ActivityPropsFn
{
    ...
    const auto num_neutral_species = mixture.numNeutralSpecies();

    // The indices of the charged and neutral species
    const auto icharged_species = mixture.indicesChargedSpecies();
    const auto ineutral_species = mixture.indicesNeutralSpecies();
