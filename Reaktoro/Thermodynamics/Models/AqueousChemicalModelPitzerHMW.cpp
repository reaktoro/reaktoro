...
{
    if(0.0 <= x && x <= 1.0)
        return interpolate(x, 0.0, 1.0, J0region1);
    if(1.0 <= x && x <= 10.0)
        return interpolate(x, 1.0, 10.0, J0region2);
    if(10.0 <= x && x <= 100.0)
        return interpolate(x, 10.0, 100.0, J0region3);
    if(100.0 <= x && x <= 1000.0)
        return interpolate(x, 100.0, 1000.0, J0region4);
    if(1000.0 <= x && x <= 10000.0)
        return interpolate(x, 1000.0, 10000.0, J0region5);
...
{
    if(0.0 <= x && x <= 1.0)
        return interpolate(x, 0.0, 1.0, J1region1);
    if(1.0 <= x && x <= 10.0)
        return interpolate(x, 1.0, 10.0, J1region2);
    if(10.0 <= x && x <= 100.0)
        return interpolate(x, 10.0, 100.0, J1region3);
    if(100.0 <= x && x <= 1000.0)
        return interpolate(x, 100.0, 1000.0, J1region4);
    if(1000.0 <= x && x <= 10000.0)
        return interpolate(x, 1000.0, 10000.0, J1region5);
...
PitzerParams::PitzerParams(const AqueousMixture& mixture)
{
    ...
    for(auto& x : temperatures)
        x = convertCelsiusToKelvin(x);
    for(auto& x : pressures)
        x = convertBarToPascal(x);
...
auto computeF(const AqueousMixtureState& state, const PitzerParams& pitzer) -> double
{
    ...
    for(unsigned i = 0; i < num_cations - 1; ++i)
        for(unsigned j = i + 1; j < num_cations; ++j)
            F += m[idx_cations[i]] * m[idx_cations[j]] * Phi_prime_cc(state, pitzer, i, j);

    // Iterate over all pairs of distinct anions
    for(unsigned i = 0; i < num_anions - 1; ++i)
        for(unsigned j = i + 1; j < num_anions; ++j)
...
auto aqueousChemicalModelPitzerHMW(const AqueousMixture& mixture) -> PhaseChemicalModel
{
    ...
    PhaseChemicalModel model = [=](PhaseChemicalModelResult& res, Temperature T, Pressure P, VectorConstRef n) mutable {
        ...
        for(unsigned X = 0; X < pitzer.idx_anions.size(); ++X) {
        ...
        for(unsigned N = 0; N < pitzer.idx_neutrals.size(); ++N) {
