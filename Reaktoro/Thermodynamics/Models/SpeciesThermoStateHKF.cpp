...
auto checkTemperatureValidityHKF(Temperature& T, const SpeciesType& species) -> void
{
    ...
    if(global::options.exception.enforce_temperature_bounds)
        Assert(T >= 0.0 && T <= hkf.Tmax,
               "Unable to calculate the "
               "thermodynamic properties of species " +
                   species.name() +
                   " using the "
                   "revised HKF equations of state.",
               "The provided temperature `" +
                   std::to_string(T.val) +
                   " K` is either negative or greater than the "
                   "maximum allowed `" +
                   std::to_string(hkf.Tmax) + " K`.");
...
auto speciesThermoStateSoluteHKF(Temperature T, Pressure P, const AqueousSpecies& species, const SpeciesElectroState& aes, const WaterElectroState& wes) -> SpeciesThermoState
{
    ...
    auto G = Gf - Sr * (T - Tr) - c1 * (T * log(T / Tr) - T + Tr) + a1 * (Pbar - Pr) + a2 * log((psi + Pbar) / (psi + Pr)) - c2 * ((1.0 / (T - theta) - 1.0 / (Tr - theta)) * (theta - T) / theta - T / (theta * theta) * log(Tr / T * (T - theta) / (Tr - theta))) + 1.0 / (T - theta) * (a3 * (Pbar - Pr) + a4 * log((psi + Pbar) / (psi + Pr))) - w * (Z + 1) + wr * (Zr + 1) + wr * Yr * (T - Tr);

    auto H = Hf + c1 * (T - Tr) - c2 * (1.0 / (T - theta) - 1.0 / (Tr - theta)) + a1 * (Pbar - Pr) + a2 * log((psi + Pbar) / (psi + Pr)) + (2.0 * T - theta) / pow(T - theta, 2) * (a3 * (Pbar - Pr) + a4 * log((psi + Pbar) / (psi + Pr))) - w * (Z + 1) + w * T * Y + T * (Z + 1) * wT + wr * (Zr + 1) - wr * Tr * Yr;

    auto S = Sr + c1 * log(T / Tr) - c2 / theta * (1.0 / (T - theta) - 1.0 / (Tr - theta) + log(Tr / T * (T - theta) / (Tr - theta)) / theta) + 1.0 / pow(T - theta, 2) * (a3 * (Pbar - Pr) + a4 * log((psi + Pbar) / (psi + Pr))) + w * Y + (Z + 1) * wT - wr * Yr;

    auto Cp = c1 + c2 / pow(T - theta, 2) - (2.0 * T / pow(T - theta, 3)) * (a3 * (Pbar - Pr) + a4 * log((psi + Pbar) / (psi + Pr))) + w * T * X + 2.0 * T * Y * wT + T * (Z + 1.0) * wTT;
