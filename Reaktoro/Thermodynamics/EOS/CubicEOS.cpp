...
auto alpha(CubicEOS::Model type) -> std::function<AlphaResult(const ThermoScalar&, double)>
{
    ...
    auto alphaPR = [](const ThermoScalar& Tr, double omega) -> AlphaResult {
        ...
        double m = omega <= 0.491 ? 0.374640 + 1.54226 * omega - 0.269920 * omega * omega : 0.379642 + 1.48503 * omega - 0.164423 * omega * omega + 0.016666 * omega * omega * omega;
        ThermoScalar sqrtTr = sqrt(Tr);
        ThermoScalar aux_val = 1.0 + m * (1.0 - sqrtTr);
        ThermoScalar aux_ddt = -0.5 * m / sqrtTr;
        ThermoScalar aux_d2dt2 = 0.25 * m / (Tr * sqrtTr);
    ...
    switch(type) {
    case CubicEOS::VanDerWaals:
        return alphaVDW;
    case CubicEOS::RedlichKwong:
        return alphaRK;
    case CubicEOS::SoaveRedlichKwong:
        return alphaSRK;
    case CubicEOS::PengRobinson:
        return alphaPR;
    default:
        return alphaPR;
    }
...
struct CubicEOS::Impl
{
    ...
    auto operator()(const ThermoScalar& T, const ThermoScalar& P, const ChemicalVector& x) -> Result
    {
        ...
        ChemicalScalar IT;
        if(epsilon != sigma)
            IT = ((ZT + sigma * betaT) / (Z + sigma * beta) - (ZT + epsilon * betaT) / (Z + epsilon * beta)) / (sigma - epsilon);
        else
            IT = I * (betaT / beta - (ZT + epsilon * betaT) / (Z + epsilon * beta));
        ...
        for(unsigned i = 0; i < nspecies; ++i) {
            ...
            ChemicalScalar Ii;
            if(epsilon != sigma)
                Ii = I + ((Zi + sigma * betai) / (Z + sigma * beta) - (Zi + epsilon * betai) / (Z + epsilon * beta)) / (sigma - epsilon);
            else
                Ii = I * (1 + betai / beta - (Zi + epsilon * betai) / (Z + epsilon * beta));
...
{
    Assert(values.size() == numSpecies(),
           "Cannot set the critical "
           "temperatures of the species in the CubicEOS object.",
           "Expecting " +
               std::to_string(numSpecies()) + " values, but only " +
               std::to_string(values.size()) + " were given.");

    Assert(minValue(values) > 0,
           "Cannot set the critical temperatures of the species "
           "in the CubicEOS object.",
           "Expecting non-zero critical "
...
auto CubicEOS::setCriticalPressures(const std::vector<double>& values) -> void
{
    Assert(values.size() == numSpecies(),
           "Cannot set the critical "
           "pressures of the species in the CubicEOS object.",
           "Expecting " +
               std::to_string(numSpecies()) + " values, but only " +
               std::to_string(values.size()) + " were given.");

    Assert(minValue(values) > 0,
           "Cannot set the critical pressures of the species "
           "in the CubicEOS object.",
           "Expecting non-zero critical "
...
{
    Assert(values.size() == numSpecies(),
           "Cannot set the acentric "
           "factors of the species in CubicEOS.",
           "Expecting " +
               std::to_string(numSpecies()) + " values, but only " +
